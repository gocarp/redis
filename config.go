...
var (
	...
	localConfigMap = maps.NewStrAnyMap(true)
)
...
func ConfigFromMap(m map[string]interface{}) (config *Config, err error) {
	config = &Config{}
	if err = conv.Scan(m, config); err != nil {
		err = errors.NewCodef(codes.CodeInvalidConfiguration, `invalid redis configuration: %#v`, m)
	}
